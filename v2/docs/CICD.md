...
| `v2-deploy.yml` | Build images → push to ACR → roll Container App revisions | Push to `v2` touching `v2/api/**` or `v2/web/**`, or manual | Yes (OIDC) |
| `v2-infra.yml` | `terraform plan` / `apply` / `destroy` on `v2/infra/azure` | Manual only | Yes (OIDC) |
